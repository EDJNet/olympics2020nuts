...
Are these *all* Olympics medalists? We now have `r all_medalists_wd_df %>% distinct(medalist_wikidata_id) %>% nrow()` and `r all_medalists_wd_df %>% distinct(event_wikidata_id) %>% nrow()` events, which correspond to the figure given in the opening line of the [Wikipedia page used as the source for this endeavour](https://en.wikipedia.org/wiki/List_of_2020_Summer_Olympics_medal_winners).
...
At last update, place of birth is present for `r place_of_birth_table$share_present` of medalists, so only `r place_of_birth_table$missing_place_of_birth` medalists out of `r place_of_birth_table$place_of_birth+place_of_birth_table$missing_place_of_birth` are missing from this map.
