...
- `tidywikidatar`, for getting data out Wikidata. It's on CRAN, and you can can install it with `install.packages("tidywikidatar")`. Make sure to have at least version 0.4, or download the latest version from GitHub. More info about the package [on its own website](https://edjnet.github.io/tidywikidatar/). 
...
o20_get_medals_from_table <- function(table) {
  ...
  if (!(length(table_colnames)==4|length(table_colnames)==7)) {
    return(NULL)
  }
...
all_medalists_df <- purrr::map_dfr(.x = all_medal_tables,
                                   .f = function(table) {
...
all_medalists_wd_nuts_df <- all_medalists_with_wd_details_df %>% 
  dplyr::left_join(
    all_medalists_with_wd_details_df %>% 
      ...
      sf::st_join(latlon2map::ll_get_nuts_eu(level = 1),
    ...
    all_medalists_with_wd_details_df %>% 
      ...
      dplyr::distinct(medalist_wikidata_id, .keep_all = TRUE) %>% 
      sf::st_as_sf(coords = c("lon","lat"), crs = 4326) %>% 
      sf::st_join(latlon2map::ll_get_nuts_eu(level = 2),
                  join = sf::st_intersects) %>% 
      st_drop_geometry() %>% 
      dplyr::transmute(medalist_wikidata_id,
                       nuts2_id = NUTS_ID,
    ...
    all_medalists_with_wd_details_df %>% 
      ...
      dplyr::distinct(medalist_wikidata_id, .keep_all = TRUE) %>% 
      sf::st_as_sf(coords = c("lon","lat"), crs = 4326) %>% 
      sf::st_join(latlon2map::ll_get_nuts_eu(level = 3),
                  join = sf::st_intersects) %>% 
      st_drop_geometry() %>% 
      dplyr::transmute(medalist_wikidata_id,
                       nuts3_id = NUTS_ID,
...
eurostat::get_eurostat(id = "tgs00096", filters = list(age = "TOTAL",
                                                       sex = "T")) %>% 
  dplyr::filter(time==as.Date("2020-01-01")) %>% 
  tidyr::drop_na() %>% 
...
gdp_by_nuts_df <- eurostat::get_eurostat(id = "nama_10r_3gdp",
    ...
    dplyr::filter(time==as.Date("2019-01-01")) %>% 
    tidyr::drop_na() %>% 
...
all_medalists_wd_nuts_pop_df <- all_medalists_wd_nuts_df %>% 
  ...
  dplyr::mutate(nuts0_id = stringr::str_extract(string = nuts1_id, pattern = "[A-Z]{2}")) %>% 
  dplyr::left_join(y = eurostat::ea_countries %>% 
  dplyr::rename(nuts0_id = code, nuts0_name = name), by = "nuts0_id") %>% 
...
all_medalists_wd_nuts_pop_df %>%
  ...
  dplyr::select(event_name, medal, medalist_name, medalist_wikidata_id, delegation_name, sex_or_gender) %>% 
...
Here is the datasets for [only medalists with place of birth in a NUTS region](medalists_nuts_only.csv).


Here is the datasets for [only medalists with no known place of birth in Wikidata](medalists_missing_place_of_birth.csv) (a good starting place to contribute to Wikidata).
